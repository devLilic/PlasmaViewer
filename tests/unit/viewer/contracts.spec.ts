...
describe('PlasmaViewer protocol', () => {
  it('clamps transforms to safe ranges', () => {
    expect(normalizeTransform({ brightness: 300, zoom: 0, panX: -200, panY: 150, flipX: true })).toEqual({
      brightness: 200, zoom: 1, panX: -100, panY: 100, flipX: true,
    })
  })
  ...
  it('accepts a valid show command', () => {
    expect(isViewerCommand({
      ...
      payload: { image: { imageId: 1, articleId: 2, title: 'Titlu', url: 'http://plasma.test/image' }, transform: normalizeTransform({}) },
    })).toBe(true)
  })
