...
FR3 is a separate frameless, fullscreen background window on the display selected for FR2. It displays the local default image using `cover`, cannot receive focus or input, and is behind FR2 whenever both windows are visible. Its toggle does not alter FR2, and `show`/`hide` continue to control FR2 only. FR2 renders the same local default image as its bottom `cover` layer and the image received from `plasma.test` above it. The FR3 transform applies to the default layer in both windows; protocol transform commands affect only the onAIR layer. While FR2 is fullscreen, FR3 is hidden temporarily and returns automatically when FR2 is windowed again. In either fullscreen or windowed mode, `hide` clears only the onAIR layer and leaves FR2 open with its default layer; the persisted windowed bounds remain unchanged and are restored when fullscreen is disabled.
